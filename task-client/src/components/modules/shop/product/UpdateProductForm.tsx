...
export default function UpdateProductForm({ product }: { product: IProduct }) {
  ...
  const form = useForm({
    defaultValues: {
      ...
      availableColors: products?.availableColors?.map((color: any) => ({
        value: color,
      })) || [{ value: "" }],

      specification: Object.entries(products?.specification || {}).map(
        ([key, value]) => ({ key, value })
      ) || [{ key: "", value: "" }],

      keyFeatures: products?.keyFeatures?.map((feature: string) => ({
        value: feature,
      })) || [{ value: "" }],
    },
  });
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      const res = await updateProduct(products?._id, formData);
